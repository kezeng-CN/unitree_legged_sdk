...
void Custom::HighCMDDown()
{

    static int motiontime = 0;
    (motiontime += 2) %= 2000;
    if (motiontime < 1000 && cmd.mode != 7)
    {
        cmd.mode = 5;
    }
    else
...
int main(void)
{
    ...
    LoopFunc loop_stdin("stdin_loop", custom.dt, boost::bind(&Custom::StdinControl, &custom));
